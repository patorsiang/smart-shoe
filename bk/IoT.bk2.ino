...
#define BALANCE_THRESHOLD 2000            // Force difference to trigger imbalance
#define MIN_IMBALANCE_TIME 30000          // 30 seconds before triggering alert
#define MQTT_ALERT_INTERVAL 5 * 60 * 1000 // 5 minutes before sending MQTT alerts
...
const unsigned long uploadInterval = 5 * 60 * 1000; // 5 minutes
...
void uploadData()
{
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    {
      payload += ",\"imbalance\":\"true\"";
      Serial.println("⚠️ [MQTT] Sending Imbalance Alert...");
    }
    else
    {
      payload += "\"imbalance\":\"false\"";
    }
...
void detectUnevenWeight(int foreForce, int heelForce)
{
  ...
  if (forceDifference > BALANCE_THRESHOLD)
  {
    ...
    // If imbalance lasts longer than MIN_IMBALANCE_TIME, confirm alert
